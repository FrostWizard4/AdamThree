import math
import random
print("AdamThree, a simple neural network designed in python. Copyright Arik Salehi 2020.")
print("Base code created by Milo Spencer-Harper 2019")
print("v2.6")
class NeuralNetwork():
	def __init__(self):
		...
		# Create a bias and set it to a random value in the rangeo of -1 to 1
...
neural_network.train(training_set_examples, number_of_iterations=100)

print("New weights after training: " + str(neural_network.weights))
print("New bias after training: " + str(neural_network.bias))
input_num = float(input("Input a new situation #: "))
# Make a prediction for a new situation
new_situation = [input_num, 0, 0]
prediction = neural_network.think(new_situation)

print("Prediction for the new set, [{:.0f},0,0] ->  ".format(input_num)  + str(prediction))
